...
SerialPort.prototype.toggleDTR = function (time, callback) {
  time = time || 100;

  var self = this;
  self.setDTR(0, function (err) {
    if (err) {
      return callback(err);
    }
    setTimeout(function () {
      self.setDTR(1, callback);
    }, time);
  })
};

SerialPort.prototype.setDTR = function (val, callback) {
  console.log('set dtr', val);
  SerialPortBinding.ioctl(this.fd, 'dtr', val, function (err) {
